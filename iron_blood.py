...
class IronBloodUniverse(AnyFateUniverse):
    ...
    def __init__(self):
        ...
        self.skip_zero_kill = self.early_stop and self.opt.get("skip_zero_kill", False)
    ...
    def end_of_university(self):
        ...
        try:
            if self.plane_floor==3:
                self.kill_count+=1
            # 开启跳过 0 杀时，本局 0 杀不写入记录
            if not (self.skip_zero_kill and self.kill_count == 0):
    ...
    def update_count(self, read=True):
        ...
        if read:
            ...
            # 开启跳过 0 杀时，本局 0 杀不增加计数
            if self.skip_zero_kill and self.kill_count == 0:
                CUS_LOGGER.debug("本局 0 杀，跳过铁血计数增加")
                return
