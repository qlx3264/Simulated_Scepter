...
class IronBloodUniverse(AnyFateUniverse):
    ...
    def __init__(self):
        ...
        self.boss_before_39_pause_enabled = self.opt.get("boss_before_39_pause_enabled", False)
        self.fate = "毁灭"
    ...
    def select_go(self):
        ...
        if self.click_text(text="选择移动目标", box=[1609, 1759, 965, 996],click=False,allow_fail=True):
            ...
            if self.next_node is not None:
                # 最终首领格前达到39战及以上时暂停
                if self.boss_before_39_pause_enabled and self.next_node["name"] == "boss" and self.kill_count >= 39:
